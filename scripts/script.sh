...
docker login -u devopsmptech -p devopsmptech
docker push devopsmptech/application:$VERSION
ssh -i /var/lib/jenkins/new.pem -o StrictHostKeyChecking=no ec2-user@172.31.82.35 docker login -u devopsmptech -p devopsmptech
